...
export const DePartamentoEchoVictima = ({dataApi}) => {
    ...
    const analisisData = () => {
        const auxData = [];
        dataApi.data.rows.map(({ Victima }) => {
            if (Victima) {
                if(Victima.DeptoLugarHecho){
                auxData.push(Victima.DeptoLugarHecho.descripcion);
                }else{
                    auxData.push('No Asignado');    
                }
            } else {
                auxData.push('Sin Asignar Victima');
            }
        });
    ...
    return (
            <Bar id="graficasLineas"
                ...
                options={{
                    ...
                    title: {
                        display: true,
                        text: 'Victima Departamento de Echo',
                        fontSize: 24
                    },
                    scales: {
                        yAxes: [{
